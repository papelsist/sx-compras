...
class Cfdi4FacturaBuilder {
    ...
    private BigDecimal baseTraslados
    ...
    def buildComprobante(){
        log.info("Generando CFDI 4.0 para factura: ${this.cxc.tipo} - ${this.cxc.documento}")
        ...
        def comprobanteFecha = DateUtils.getCfdiDate(new Date())
        println "La fecha del comprobante es: "+ comprobanteFecha
        println DateUtils.getXmlGregorianCalendar(new Date())

        //comprobante.setFecha(DateUtils.getXmlGregorianCalendar(new Date()))
        comprobante.setFecha(DateUtils.getCfdiDate(new Date()))
        comprobante.moneda =  V4CfdiUtils.getMonedaCode(cxc.moneda)
    ...
    def buildReceptor(){
        ...
        receptor.domicilioFiscalReceptor = venta.cliente.direccion.codigoPostal
        /* TODO- Regimen */
    ...
    def buildConceptos(){
        ...
        this.venta.partidas.each { det ->
            ...
            concepto.with {
                ...
                this.baseTraslados += det.subtotal
