...
@GrailsCompileStatic
@Component
class CobranzaDepositosJurTask implements  AsientoBuilder {
    ...
    String getSelect() {
        ...
        FROM (  
        SELECT 
        (case when M.por_identificar is true then 'COB_TRANSF_JUR_xIDENT' else 'COB_TRANSF_JUR' end) as asiento,b.moneda,b.tipo_de_cambio tc,s.nombre sucursal, s.clave suc
        ...
        where M.fecha='@FECHA' and date(b.primera_aplicacion)=a.fecha and b.tipo ='JUR'
        UNION
        SELECT 
        (case when M.por_identificar is true then 'COB_DEP_EFE_JUR_xIDENT' else 'COB_DEP_EFE_JUR' end) as asiento,b.moneda,b.tipo_de_cambio tc,s.nombre sucursal, s.clave suc
        ,concat((case when M.por_identificar is true then '205-0002-' else '102-0001-' end),z.sub_cuenta_operativa,'-0000') as cta_contable,(case when m.por_identificar is true then '208-0003-0000-0000' else '000-0000-0000-0000' end) cta_contable2
        ...
        join sucursal s on(b.sucursal_id=s.id) join aplicacion_de_cobro a on(a.cobro_id=b.id) join cuenta_por_cobrar c on(a.cuenta_por_cobrar_id=c.id) left join cfdi i on(c.cfdi_id=i.id)
        where M.fecha='@FECHA' and date(b.primera_aplicacion)=a.fecha and b.tipo ='JUR' and x.total_efectivo>0 and x.total_efectivo>=x.total_cheque 
        UNION
        SELECT 
        (case when M.por_identificar is true then 'COB_DEP_CHE_JUR_xIDENT' else 'COB_DEP_CHE_JUR' end) as asiento,b.moneda,b.tipo_de_cambio tc,s.nombre sucursal, s.clave suc
        ,concat((case when M.por_identificar is true then '205-0002-' else '102-0001-' end),z.sub_cuenta_operativa,'-0000') as cta_contable,(case when m.por_identificar is true then '208-0003-0000-0000' else '000-0000-0000-0000' end) cta_contable2
        ...
        FROM movimiento_de_cuenta m  join cuenta_de_banco z on(m.cuenta_id=z.id) join cobro_deposito x on(x.ingreso_id=m.id) join cobro b on(x.cobro_id=b.id)  join cliente t on(b.cliente_id=t.id)
        join sucursal s on(b.sucursal_id=s.id) join aplicacion_de_cobro a on(a.cobro_id=b.id) join cuenta_por_cobrar c on(a.cuenta_por_cobrar_id=c.id) left join cfdi i on(c.cfdi_id=i.id)
        where M.fecha='@FECHA' and date(b.primera_aplicacion)=a.fecha and b.tipo ='JUR' and x.total_cheque>0  and x.total_efectivo<x.total_cheque
        ) as x
        """
        return res
    }
