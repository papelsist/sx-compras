...
@GrailsCompileStatic
@Component
class CobranzaTarjetaCheTask implements  AsientoBuilder {
    ...
    String getSelect() {
        ...
        FROM (                                                                    
        SELECT 
        concat('COB_TARJ_',(case when x.debito_credito is true then 'DEB' when x.visa_master is false then 'AME' else 'CRE' end),'_CHE') as asiento,f.id origen_gpo,x.debito_credito,(case when x.debito_credito is true then 'DEBITO' else 'CREDITO' end)  documentoTipo,f.corte fecha,f.folio documento_gpo,b.moneda,b.tipo_de_cambio tc ,f.total total_gpo
        ...
        ,concat('106-0001-',(SELECT x.cuenta_operativa FROM cuenta_operativa_cliente x where x.cliente_id=t.id ),'-0000') cta_contable_fac       
        ...
        where a.fecha='@FECHA' and a.fecha=date(b.primera_aplicacion) and b.tipo='CHE' and j.tipo like '%INGRESO%'        
        ) as x   
