...
class FacturistaEstadoDeCuentaService implements  LogUser, FolioLog {
    ...
    def generarNotasDeCargoPorIntereses(Date fecha, String comentario = 'INTERESES POR PRESTAMO') {
        ...
        def res = generarNotaDeCargoRepse( fecha, comentario)
        /* facturistas.each { f ->
    ...
    @Transactional
    NotaDeCargo generarNotaDeCargoRepse(Date corte = new Date(), String comentario) {
        log.info('Generando N de Cargo por intereses para  Corte: {}, Desc: {}', corte, comentario)
        ...
        //***** HARD CODED CLIENTE REPSE INTERMINER ****//
        nc.cliente = Cliente.where{rfc == "INT240614FS3"}.find()
        //***** HARD CODED CLIENTE REPSE INTERMINER ****//
