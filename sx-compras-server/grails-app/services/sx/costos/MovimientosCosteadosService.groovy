...
SELECT A.CLAVE as clave ,x.de_linea as deLinea,A.DESCRIPCION as descripcion ,L.LINEA as linea, C.CLASE as clase, M.MARCA as marca 
    ...
    ,ROUND(SUM(CASE WHEN A.TIPO IN('SRE','STR') THEN A.SALDO ELSE 0 END) ,3) AS trsSalUni
    ,ROUND(SUM(CASE WHEN A.TIPO IN('SRE','STR') THEN A.COSTO ELSE 0 END) ,2) AS trsSalCosto
    ,ROUND(SUM(CASE WHEN A.TIPO IN('ERE','ETR') THEN A.SALDO ELSE 0 END) ,3) AS trsEntUni
    ,ROUND(SUM(CASE WHEN A.TIPO IN('ERE','ETR') THEN A.COSTO ELSE 0 END) ,2) AS trsEntCosto
    ...
    FROM (      
	   ...
	   FROM existencia i join producto p on(p.id=i.producto_id) JOIN sucursal s on(i.sucursal_id=s.id) where p.inventariable is true and I.ANIO= ? AND I.MES = ?  AND I.CLAVE LIKE '%'         
	   	union
	   ...
	   FROM existencia i join producto p on(p.id=i.producto_id) JOIN sucursal s on(i.sucursal_id=s.id) where p.inventariable is true and I.ANIO= ? AND I.MES = ?  AND I.CLAVE LIKE '%' 	   	   
        ...
        where p.inventariable is true and  YEAR(I.FECHA)= ? AND MONTH(I.FECHA) = ? 
        ...
        ,(CASE WHEN I.TIPO IN('COM') THEN '02 COMPRAS' WHEN I.TIPO IN('AJU','CIM','CIS','DEC','MER','RMC') THEN '03 GASTO' WHEN I.TIPO IN('VIR','OIM') THEN '04 PRODUCTO'
         WHEN I.TIPO IN('REC','TRS') THEN '05 TRANSFORM' 
        ...
        from inventario I  join producto p on(p.id=i.producto_id) JOIN sucursal s on(i.sucursal_id=s.id)
        where p.inventariable is true and  YEAR(I.FECHA)= ? AND MONTH(I.FECHA) = ? AND I.CLAVE LIKE '%'           
        ) as a       
        group by 1,2,6,7,12                  
    ) AS A 
    ...
    def movimientos(Integer ejercicio, Integer mes) {
        return getLocalRows(MOVIMIENTOS_SQL, [ejercicio, mes, ejercicio, mes, ejercicio, mes, ejercicio, mes, ejercicio, mes])
    }
