        ...
        switch(tipo){
            case 'GENERAR':
                    select = """ 
                            ...
                            FROM (
                            SELECT case when PD.DOCUMENTO_TIPO='CON' AND PD.ASIENTO LIKE '%COMISION%TARJ%' AND PD.REFERENCIA like '%AMEX%' THEN 'AMERICAN EXPRESS COMPANY MEXICO SA DE CV' when PD.DOCUMENTO_TIPO='CON' AND PD.ASIENTO LIKE '%COMISION%TARJ%' AND PD.REFERENCIA NOT like '%AMEX%' THEN 'BANCO NACIONAL DE MEXICO SA' 
                            when pd.referencia is null and pd.referencia2 is not null then pd.referencia2 ELSE PD.REFERENCIA END AS PROVEEDOR
            ...
            case 'LAYOUT':
                ...
                select = """
                        select sum(pagos1516) as pagos1516, sum(pagos15) as pagos15, sum(iva_pagado1516) as ivaPagado1516, sum(pagos1011) as pagos1011, sum(pagos10) as pagos10, sum(pagos_frontera) as pagosFrontera,sum(iva_pagado1011) as ivaPagado1011
                        ,sum(iva_pagado_frontera) as ivaPagadoFrontera, sum(pagos_importacion) as pagosImportacion, sum(iva_pagado_importacion1516) as ivaPagadoImportacion1516,sum(pagos_importacion1011) as pagosImportacion1011
                        , sum(iva_pagado_importacion1011) as ivaPagadoImportacion1011,sum(pagos_importacion_sin_iva) as pagosImportacionSinIva,sum(pagos_tasa0) as pagosTasa0, sum(pagos_sin_iva) as pagosSinIva,sum(iva_retenido_contribuyente) as ivaRetenidoContribuyente, sum(iva_notas) as ivaNotas
                        from Diot where mes= ? and ejercicio = ? and pagos1516 < 50000
                    """
                return select
