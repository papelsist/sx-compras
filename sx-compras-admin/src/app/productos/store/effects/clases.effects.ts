...
export class ClasesEffects {
  ...
  @Effect()
  createClase$ = this.actions$.pipe(
    ofType(fromClases.CREATE_CLASE),
    map((action: fromClases.CreateClase) => action.payload),
    switchMap(clase => {
      return this.service.save(clase).pipe(
  ...
  @Effect()
  updateClase$ = this.actions$.pipe(
    ofType(fromClases.UPDATE_CLASE),
    map((action: fromClases.UpdateClase) => action.payload),
    switchMap(clase => {
      return this.service.update(clase).pipe(
