...
@Component({
  selector: 'sx-analisis-de-notas',
  templateUrl: './analisis-de-notas.component.html',
  styleUrls: ['./analisis-de-notas.component.scss']
})
export class AnalisisDeNotasComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.notas$ = this.store.pipe(select(fromStore.getDevluciones));
  }

  onSelect(event: NotaDeCreditoCxP) {
    this.store.dispatch(
      new fromRoot.Go({ path: ['cxp/analisisDeNotas/', event.id] })
    );
  }
